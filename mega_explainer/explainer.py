...
from mega_explainer.lime_explainer import Lime
from mega_explainer.perturbation_methods import NormalPerturbation
from mega_explainer.shap_explainer import SHAPExplainer
from mega_explainer.anchor_explainer import AnchorExplainer  # Make sure this is the correct import path
...
class Explainer:
    ...
    def __init__(self,
        ...
        # Add Anchor explanation
        anchor_explainer = AnchorExplainer(self.model, self.data, self.feature_names, discrete_features)
        available_explanations["anchor"] = anchor_explainer
    ...
    def _compute_faithfulness_auc(self, data, explanation, c_label, k, metric="topk"):
        ...
        # Filter out non-numeric columns if necessary
            explanation = np.array([float(x) if isinstance(x, (int, float)) else 0.0 for x in explanation])
        elif isinstance(explanation, list):
        # Convert list to a numeric array, handling non-numeric values
            explanation = np.array([float(x) if isinstance(x, (int, float)) else 0.0 for x in explanation])

        explanation_tensor = torch.tensor(np.abs(explanation)) if isinstance(explanation, np.ndarray) else explanation


        for k_i in k:
            # Construct original mask as all true (i.e., all indices are masked and non are perturbed)
            top_k_map = torch.tensor([True] * len(explanation_tensor), dtype=torch.bool)

            # Unmask topk instances
            top_k_map[torch.topk(np.abs(explanation_tensor), k=k_i).indices] = False
    ...
    def explain_instance(self,
        ...
        for method in self.explanation_methods.keys():
            ...
            cur_expl, score = cur_explainer.get_explanation(formatted_data,
                                                            label=label)
            cur_expl = np.array(cur_expl)
            # Check if the array can be squeezed, i.e., it has a dimension of size 1
            if cur_expl.ndim > 1 and cur_expl.shape[0] == 1:
                cur_expl = cur_expl.squeeze(0)  # Remove the first dimension if its size is 1
            elif cur_expl.ndim == 1:
                # No need to squeeze if it already has 1 dimension
                pass
            else:
                # If cur_expl cannot be squeezed, just keep it as is
                pass
            explanations[method] = cur_expl
            scores[method] = score
        ...
        if len(fidelity_scores_topk) >= 2:
         ...
         # Create Explanation objects for each method
        explanations_with_scores = [
            Explanation(method_name=method, explanation_data=explanations[method],
                        faithfulness_score=fidelity_scores_topk[method])
            for method in fidelity_scores_topk
        ]
        
        # Sort explanations by faithfulness score
        sorter = FaithfulnessSorter(explanations_with_scores)
        sorted_explanations_objects = sorter.sort_by_faithfulness()

    # Convert sorted explanations to MegaExplanation format
        sorted_explanations = [
            MegaExplanation(
                list_exp=exp.explanation_data,
                score=scores[exp.method_name],
                label=label,
                best_explanation_type=exp.method_name,
                agree=True  # Assume agreement if we're sorting by faithfulness
            )
            for exp in sorted_explanations_objects
        ]

        # Return sorted explanations and optionally fidelity scores
        if return_fidelities:
            return sorted_explanations, fidelity_scores_topk
        else:
    ...
    def compute_stability_topk(self, data, baseline_explanation, explainer, label, top_k, num_perturbations=100):
        ...
        """
        if isinstance(baseline_explanation, np.ndarray):
            baseline_explanation = torch.tensor(baseline_explanation, dtype=torch.float32)
        elif isinstance(baseline_explanation, list):
            baseline_explanation = torch.tensor(baseline_explanation, dtype=torch.float32)
